...
uint8_t DS18X20_start_meas(uint8_t with_power_extern, uint8_t id[]) {
	OW_Reset(); //**
	if (OW_InputPinState()) { // only send if bus is "idle" = high
...
void TempOut(uint8_t* resultMeasure) {
	...
	if (decicelsius / 100) {
		tempLcd[0] = sign;
		tempLcd[1] = (decicelsius / 100) + '0';
		tempLcd[2] = (decicelsius / 10) % 10 + '0';
	} else if (decicelsius / 10) {
		tempLcd[0] = ' ';
		tempLcd[1] = sign;
		tempLcd[2] = (decicelsius / 10) + '0';
	} else {
		tempLcd[0] = ' ';
		tempLcd[1] = ' ';
		tempLcd[2] = sign;
	}
	tempLcd[4] = decicelsius  % 10 + '0';
}
...
{	OW_Reset(); //**
	...
	{ // only send if bus is "idle" = high
		...
		OW_ByteWR(th);
		OW_ByteWR(tl);
		if (id[0] == DS18B20_ID) OW_ByteWR(conf);// config avail. on B20 only
		return DS18X20_OK;
	}
	else
	{
...
{	uint8_t i;
	...
	{ // only send if bus is "idle" = high
		OW_Command( DS18X20_READ, id );
		for ( i=0; i< DS18X20_SP_SIZE; i++ ) sp[i]=OW_ByteRD();
		return DS18X20_OK;
	}
	else
	{
